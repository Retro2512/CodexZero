        ...
        env.update(
            {
                ...
                "CODEX_MODEL_INSTRUCTIONS_PATH": str(lean_prompt),
            }
        )
        kwargs["config_toml"] = CZ_TOML
...
def main() -> int:
    ...
    if checkpoint.exists():
        ...
        if state["dependencies"] != manifest or state["task_order"] != selected:
            raise SystemExit("Checkpoint dependencies or task order changed")
    else:
        state = {
            "schema": "codexzero-deepswe-five-way-run-v1",
            "created_at": now(),
            "model": MODEL,
            "reasoning_effort": EFFORT,
            "seed": SEED,
        ...
        if args.seed_checkpoint is not None:
            ...
            if (
                ...
                raise SystemExit("Seed checkpoint model configuration changed")
