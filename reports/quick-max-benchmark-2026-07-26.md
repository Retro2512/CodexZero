...
| Previous Tura Balanced result | pass | 113.0 s | 72,682 | 12,522 | 3,705 | 76,387 | not comparable | 4 model-visible / 25 internal | 0.2038 |
...
Compared with the previous Tura result, new Max finished 48.3% sooner and had a 12.0% lower weighted proxy, but used 19.6% more total tokens. Tura retained the uncached-input advantage; CodexZero retained the output-token, latency, and weighted-cost advantages.
...
This is one run per cell, so the differences are directional rather than statistically stable. The Tura number is the prior result for the same scenario, not a new interleaved run.
