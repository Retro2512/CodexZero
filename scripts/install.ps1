...
if (!$CliOnly -and ($Desktop -or (Test-Path -LiteralPath (Join-Path $PackageRoot 'CodexZero.exe')))) {
...
if (Test-Path -LiteralPath (Join-Path $sourceRoot 'assets\provider-settings.html')) {
    ...
    Get-ChildItem -LiteralPath (Join-Path $sourceRoot 'assets') -File | Where-Object { $_.Name -in @('provider-settings.html', 'native-cache-ui.mjs', 'model-pricing.mjs', 'sidebar-performance.mjs', 'transcript-retention.mjs') -or $_.Name -like 'native-provider-*' -or $_.Name -like 'native-sidebar-*' -or $_.Name -like 'codexzero.*' } | Copy-Item -Destination (Join-Path $appRoot 'assets') -Force
}
